...
public class RevistaBD {
	...
	public ArrayList<Revista> consultaNome(Consulta consulta) {

		ArrayList<Revista> revista = new ArrayList<Revista>();
		try {
			ps = this.connection.prepareStatement("SELECT id, nome, ano, mes, qtdpg FROM REVISTA WHERE nome LIKE '%"+consulta.getNome()+"%'");
		    rs = ps.executeQuery();
